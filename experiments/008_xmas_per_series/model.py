"""Per-series Christmas-window multipliers, on top of 007_bf_per_series.

007 proved that event appetite is series-specific and persistent: per-series
BF multipliers cut Nov-13/20 from 0.39 to 0.29 with bias unchanged. The
Dec-18 cutoff (worst overall, 0.49, bias +0.14) is likely the same
composition story around the year boundary: gift-driven merchants die after
the last-order deadline while replenishment-driven ones keep shipping
through the lull.

One change over 007: the identical shrunk actual-vs-expected multiplier,
computed over the Christmas-window group (dec20..dec30 day-types, Dec 24-26
holidays, nye, Jan 1) and applied on those days. BF keeps its own
multiplier from 007.
"""
