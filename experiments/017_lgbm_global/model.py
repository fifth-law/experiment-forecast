"""Global LightGBM over the calendar/level feature space.

The hand-tuned multiplicative stack (015, wMAPE 0.2232) encodes network-
pooled factors per calendar type. Its residual rocks are interaction-
shaped: series x day-type appetite (008 helped little), series x lead
decay, type x recent-level. A gradient-boosted model over the SAME
knowledge - day-type ids, lead, weekday, series identity, clean rolling
levels, same-weekday lags - can learn those interactions directly from
2021-2022 history at each cutoff.

Design (leakage-safe by construction):
  - one model trained per cutoff on simulated forecast rows: every past
    Monday r (>= train_start + 120d, <= cutoff - 14d) x lead 1..14 x
    series, features computed strictly from data <= r;
  - target = y / L14(r) (relative, comparable across series), sample
    weight = L14(r) so the L1 objective approximates pooled wMAPE;
  - prediction rows are the same features with r = cutoff;
  - deterministic: fixed seed, deterministic=True, force_row_wise=True,
    fixed thread count, fixed boosting rounds (no early stopping).

Justification for the heavier dep (journal 016): the cheap single-
mechanism well is exhausted; remaining error is n=2 factor drift and
interactions. lightgbm is already a project extra.
"""
...
class Forecaster:
    def fit_predict(self, history, cutoff, horizon, series_meta):
        ...
        L14 = Wc.rolling(14, min_periods=5).mean()
