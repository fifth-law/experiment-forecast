"""Per-series vacation multipliers, on top of 011_summer_weeks.

011's pooled damp factors fixed the aggregate vacation bias, but the
May/summer cutoffs keep residual wMAPE with shrunken bias - the 007
composition signature. Vacation appetite is series-specific and should be
persistent: a B2B-ish account collapses in week 28-30 while a consumer
webshop barely dips.

One change over 011: a per-series vacation multiplier, exactly the 007
mechanism over all damp days (asc + summer clusters): the series' actual
volume over past damp days vs its expectation (per-series clean base
before each cluster anchor x the pooled damp factor), shrunk toward 1
with k=500, clip [0.3, 2.0]. Applied on damp-day targets only:

    yhat = shape * level * damp[type] * vac_mult[series]

Level-window deflation stays pooled-only (dividing by per-series
multipliers would reintroduce 009's drift-import problem for the extreme
series).
"""
...
PARAMS = {
    "kind": "vac_per_series",
    "vac_mult_shrinkage_k": 500,
    "vac_mult_clip": [0.3, 2.0],
...
class Forecaster:
    def fit_predict(self, history, cutoff, horizon, series_meta):
        ...
        # per-series vacation multiplier over all damp days: actual vs
        # (per-series clean base before the cluster anchor x pooled factor)
        vac_actual = pd.Series(0.0, index=wide.columns)
        vac_expected = pd.Series(0.0, index=wide.columns)
        anchor_base = {}
        damp_day_index = pd.Index(list(day_damp.keys()))
        for _, gmap in groups:
            for day, (name, anchor) in sorted(gmap.items()):
                if day not in wide.index or name not in damp_factors:
                    continue
                if anchor not in anchor_base:
                    rows = clean[
                        (clean.index >= anchor - pd.Timedelta(days=SPECIAL_LEVEL_WINDOW_DAYS))
                        & (clean.index < anchor)
                        & (~clean.index.isin(damp_day_index))
                    ]
                    anchor_base[anchor] = rows.mean() if len(rows) >= 10 else None
                base = anchor_base[anchor]
                if base is None:
                    continue
                vac_actual += wide.loc[day]
                vac_expected += base * damp_factors[name]
        vac_mult = ((vac_actual + VAC_MULT_K) / (vac_expected + VAC_MULT_K)).clip(*VAC_MULT_CLIP)
        ...
        for target in targets:
            if target in smap:
                ...
                damp_name = day_damp.get(target)
                if damp_name is not None:
                    yhat = yhat * damp_factors.get(damp_name, 1.0) * vac_mult
