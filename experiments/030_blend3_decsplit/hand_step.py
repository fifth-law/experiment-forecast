...
PARAMS = {
    "kind": "hand_step_decsplit",
    "xmas_multiplier_split": ["dec_wind", "dec_lull"],
    "recency_half_life_days": 365.25,
    "recency_scope": "year-boundary responses only",
    "step_tau": 0.18,
...
RECENCY_HALF_LIFE_DAYS = 365.25  # applies ONLY to year-boundary responses (026: drift is
# response-specific - BF/holiday appetites are year-stable, the xmas window is not)

def _drifting(name: str) -> bool:
    return (
        name.startswith("dec")
        or name.startswith("runup")
        or name == "jan_wk1"
        or name in {"xmas_eve", "nye", "Juledag", "Anden juledag", "Nytårsdag"}
    )
...
WIND_NAMES = {"dec20", "dec21", "dec22", "dec23", "xmas_eve"}
LULL_NAMES = {"dec27", "dec28", "dec29", "dec30", "nye", "Nytårsdag", "Juledag", "Anden juledag"}
    ...
    def fit_predict(self, history, cutoff, horizon, series_meta):
        ...
        for day, name in sorted(smap.items()):
            ...
            if len(base) < 10 or base.mean() <= 0:
                continue
            w = 0.5 ** ((cutoff - day).days / RECENCY_HALF_LIFE_DAYS) if _drifting(name) else 1.0
        ...
        group_actual = {g: pd.Series(0.0, index=wide.columns) for g in ("bf", "dec_wind", "dec_lull")}
        group_expected = {g: pd.Series(0.0, index=wide.columns) for g in ("bf", "dec_wind", "dec_lull")}
        for day, name in sorted(smap.items()):
            ...
            if len(base_rows) < 10:
                continue
            w = 0.5 ** ((cutoff - day).days / RECENCY_HALF_LIFE_DAYS) if group != "bf" else 1.0
