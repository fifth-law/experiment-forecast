"""017_lgbm_global with the base-level zero-hole fixed.

019's diagnostics caught 017 predicting ZERO for the whole Jan-02
horizon: the base level L14 is rolling(14, min_periods=5) over CLEAN
days, and Dec 20 - Jan 1 are all special, so at a Jan-02 cutoff the
base is NaN and the base>1 guard zeroes every series (a silent ~1.0-
wMAPE cutoff).

One change over 017: the base becomes a fallback cascade
    base = L14 -> L28 -> L56  (first with enough clean days wins)
used everywhere L14 was used (base feature, ratio denominators, target
scaling, prediction rescale). At Jan-02 the L28 window still holds the
Dec 6-19 clean run-up days, so the base exists (slightly high for
January, which the day-type features can correct) instead of zero.
Everything else is byte-identical to 017.
"""
