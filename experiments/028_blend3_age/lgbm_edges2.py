...
PARAMS = {
    "kind": "lgbm_edges_cap_age",
    "age_features": ["age_days", "active_60"],
...
class Forecaster:
    def fit_predict(self, history, cutoff, horizon, series_meta):
        ...
        has_any = W.gt(0).any()
        first_nz = W.gt(0).idxmax()
        active_60 = W.gt(0).rolling(60, min_periods=1).mean()
        ...
        def build_rows(refs: pd.DatetimeIndex, with_target: bool):
            ...
            for lead in range(1, 15):
                ...
                ref_days = refs.values.astype("datetime64[D]").astype("int64")
                nz_days = first_nz.values.astype("datetime64[D]").astype("int64")
                age = pd.DataFrame(
                    np.subtract.outer(ref_days, nz_days), index=T, columns=W.columns
                ).clip(0, 730)
                age.loc[:, ~has_any] = -1
                feats["age_days"] = age
                feats["active_60"] = active_60.loc[refs].set_axis(T)
