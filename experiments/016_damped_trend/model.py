"""Damped per-series local trend, on top of 015_jan_week1.

With the calendar stack in place the remaining scattered biases look like
level lag: autumn-ramp and post-dip-recovery cutoffs run -0.10..-0.16
(the 14-day level trails a climb), a few spring cutoffs run +0.08..+0.12
(it trails a decline). The 14-day window's center sits ~7 days before the
cutoff, and targets sit 1-14 days after it.

One change over 015: a weekly growth ratio per series,
    g = level_last7 / level_prev7        (both shape/damp-deflated),
shrunk hard toward the (volume-weighted) robust middle by
    g_eff = 1 + PHI * w * (g - 1),  w = vol7 / (vol7 + 500),  PHI = 0.5,
g clipped to [0.85, 1.15] before shrinking, and applied to NORMAL-day
predictions only as yhat *= g_eff ** (lead / 7). Max effect at lead 14 is
g_eff^2 (a few percent for typical shrunk values). Special days keep the
28-day special_level convention untouched.
"""
...
class Forecaster:
    def fit_predict(self, history, cutoff, horizon, series_meta):
        ...
        # Accumulated per 7-day half so a damped weekly growth ratio can be read.
        ...
        # damped per-series weekly trend, hard-shrunk toward flat
        l_recent = (num["recent"] / den["recent"]).where(den["recent"] > 0, 0.0).fillna(0.0)
        l_prev = (num["prev"] / den["prev"]).where(den["prev"] > 0, 0.0).fillna(0.0)
        g_raw = (l_recent / l_prev).where((l_prev > 0) & (l_recent > 0), 1.0).fillna(1.0)
        g_clipped = g_raw.clip(*TREND_RATIO_CLIP)
        vol_weight = num["recent"] / (num["recent"] + TREND_VOL_K)
        g_eff = 1.0 + TREND_PHI * vol_weight * (g_clipped - 1.0)
        ...
        for target in targets:
            if target in smap:
                ...
                lead = (target - cutoff).days
                yhat = shape[target.weekday()] * level * g_eff ** (lead / 7.0)
