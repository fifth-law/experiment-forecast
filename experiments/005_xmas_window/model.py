"""Christmas-window day-types, on top of 004_black_friday.

The dominant residual is the year-boundary window: at the Dec-18 cutoff
(worst in 004, wMAPE 0.67, bias +0.41) the non-holiday days Dec 20-23
(pre-Christmas wind-down: last-order deadlines have passed) and Dec 27-30
(inter-holiday lull) are still forecast at full run-up volume.

One change over 004: those eight fixed dates become special-day types
(dec20..dec23, dec27..dec30), each with its own learned level-relative
factor, exactly like the public holidays. They migrate weekdays year over
year, which the level-relative factor convention already handles.

Known accepted risk: with Dec 20-30 special, the 14-day level window at a
Jan-02 cutoff contains almost no clean days (Jan 2 alone), so that cutoff's
level estimate gets noisy. Deliberately not patched here - if it regresses,
that is the next experiment's clean target.
"""
