"""December run-up weeks as up-factors, on top of 011_summer_weeks.

Nov-27 (0.31, bias -0.26) is the last big directional miss: with BF week
special, a post-BF cutoff's level window holds only pre-BF days, and the
December run-up regime (elevated volume from Cyber Monday to the
last-order deadline ~Dec 19) is invisible. 009 tried to see it *through*
the BF spike and failed on intensity drift; the run-up itself, however,
is a mild (~1.1-1.3x) calendar-anchored lift - exactly what the damp
machinery handles safely.

One change over 011: a third damp group `runup` with blocks Nov 28-Dec 5
(runup_w3), Dec 6-12 (runup_w2), Dec 13-19 (runup_w1, deadline week),
anchored on the pre-BF Monday with base [anchor-28, anchor), clip
(0.8, 2.0) since these are up-factors. Applied like every damp group: on
normal-day targets and as level-window deflation (run-up days deflate
back to pre-BF terms, keeping every December cutoff on one convention).
"""
...
PARAMS = {
    "kind": "dec_runup",
    "damp_groups": ["asc_wk0-2", "sum_wk25-32", "runup_w1-3"],
    "damp_clip": [0.5, 1.3],
    "runup_clip": [0.8, 2.0],
...
def damp_groups(first_year: int, last_year: int, smap: dict) -> list:
    ...
    return [("asc", asc, DAMP_CLIP), ("sum", summer, DAMP_CLIP), ("runup", runup, RUNUP_CLIP)]
